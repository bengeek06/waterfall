...
If a finding might be a backend/environment artifact rather than a genuine frontend defect (e.g. an error state that only triggers because the API is down), rule that out before reporting it as a frontend bug:
