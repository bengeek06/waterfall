...
def _cost_code_valid(db: Session, project: MsProject, cost_code_id: int | None) -> bool:
    ...
    Since E14-07 (#333) removed the devis-grid create/update routes, the import is the
    only caller of ``resolve_cost_code_id`` left -- and it always stages through this
    mirror first, which turns that function's own two refusals ("does not belong to
    project", "is not active") into defensive duplicates on the only live path: an
    invalid code is already a ``*_COST_CODE_INVALID`` blocking issue here, so the apply
    never sees one. Two formulations of one rule, kept as-is deliberately: collapsing
    them means either staging calling a raising resolver (a bare 400 instead of the
    structured 409 #70 requires) or apply trusting staging blindly. Pinned by
    ``test_reconciliation_import_deactivated_cost_code_is_blocking_before_the_apply_guard``.
    """
