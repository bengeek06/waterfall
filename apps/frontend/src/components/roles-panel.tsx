    ...
    <Card className="mt-4">
      ...
      <CardContent className="grid gap-6">
        <form onSubmit={props.onSubmit} className="grid gap-4 sm:grid-cols-2">
          <div className="grid gap-2"><Label htmlFor="role-code">Code</Label><Input id="role-code" value={props.roleCode} onChange={(event) => props.onCodeChange(event.target.value)} required /></div>
        ...
        {props.selectedRoles.length ? <ul className="grid divide-y rounded-md border">{props.selectedRoles.map((role) => <li key={role.id} className="flex flex-wrap items-center gap-2 px-3 py-2"><span className="font-medium">{role.code}</span><span>{role.name}</span><Badge variant="outline">{props.categoryNames.get(role.cost_category_id) ?? "?"}</Badge></li>)}</ul> : null}
      </CardContent>
    </Card>
  );
}
